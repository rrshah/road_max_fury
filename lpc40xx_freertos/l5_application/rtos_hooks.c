...
/**
 * Some of the comments indicate when we need the function, such as '#if (0 !=
 * configCHECK_FOR_STACK_OVERFLOW)' Instead of plaguing the code with a bunch of
 * #ifdef, we let the link time optimizer remove the function from the compiled
 * binary
 */
...
/// Called upon each interrupt that invokes the FreeRTOS tick handler: #if (0 !=
/// configUSE_TICK_HOOK)
