...
void startup__initialize_interrupts(void) {
  /* All peripherals should set their priority lower than (which is higher
   * number) configMAX_SYSCALL_INTERRUPT_PRIORITY set at FreeRTOSConfig.h
   *
   * Should be the same as (configMAX_SYSCALL_INTERRUPT_PRIORITY >>
   * configPRIO_BITS_NOT_IMPLEMENTED) + 1;
   *
  ...
  /* Note 1: enabling of IRQ or FIQ is not needed because interrupts are enabled
   * so we can use them without FreeRTOS Note 2: We should not alter base
   * priority register as the FreeRTOS API will alter it.
   *
