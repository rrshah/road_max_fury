...
// You can use the 'argument' to possible route output messages to different
output interfaces static void app_cli_output_function(app_cli__argument_t
argument, const char *string) { printf("%s", string);
}

static app_cli_status_e app_cli__hello_handler(app_cli__argument_t argument,
                                               sl_string_t
user_input_minus_command_name, app_cli__print_string_function
output_print_string) { sl_string_t output_string =
user_input_minus_command_name;  // re-use the string
...
"responds back with 'hello world'", .app_cli_handler = app_cli__hello_handler};
 ...
 * @param argument This is the same argument coming down from
 ...
 * Since the commands is stored as a linked list, we also have the
 * 'pointer_of_next_command' link
 */
typedef struct app_cli__command_s {
   ...
   * A CLI handler function receives the command arguments. For example, if the
   * command is "rf foo bar" then the command_arguments is the string with the
   * command name stripped, so "foo bar"
   *
   * @param output_function The output is the interface that should be used to
   * provide response
   *
 ...
 * If the full command name does not yield a handler, then 'short commands'
 * matcher is used For example, if the command is 'memory' and user types 'mem'
 * with this parameter being the value of 3, then the CLI will match the short
 * command with its handler. Note that this only occurs when the full
 * command-name did not match any registered handler.
 *
 * @param output_print_string
 * This function is used to output CLI data; it is also given to each CLI
 * handler to output its data
 *
 ...
 *
 * @post CLI will match the input command with one of its handler, and invoke
 * its callback Otherwise CLI will return without calling and functions and
 * outputs an error message
 *
 * @param cli_argument
 * This argument is not used by the CLI itself and it is a pass-through argument
 * to CLI handlers and to app_cli__print_string_function. It can be used to
 * identify input source and output the response to different channels, such as
 * UART0 vs. UART1
 *
 * @param input_string The design of the function is such that it allows the
 * user to buffer the input string from any input source, and the CLI module is
 * agnostic to where the data came from
 *
 * @note The input_string parameter is re-used, manipulated, and is basically
 * destroyed ... Since this string is re-used for output, CLI itself needs
