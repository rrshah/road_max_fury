...
/// Uses polled version to enforce that the data goes out (the cost is that it
/// eats our CPU)
...
int _kill(int pid, int sig) {
  system_calls__print_and_halt("ERROR: Unexpected system call to _kill(). "
                               "Behavior for this system is not defined");
  return 0;
}
 ...
 */
void _exit(int code) {
  system_calls__print_and_halt("ERROR: Unexpected system call to _exit(). "
                               "Behavior for this system is not defined");
  while (1) {
...
int _read(int file_descriptor, char *ptr, int len) {
  ...
  if (_isatty(file_descriptor)) {
    ...
    if (uart__polled_get(system_calls__uart_type, &ptr[0])) {
      // We deliberately just read 1 char, and let the libc re-invoke _read()
      // again as needed
      ++bytes_read;
    }
  } else {
