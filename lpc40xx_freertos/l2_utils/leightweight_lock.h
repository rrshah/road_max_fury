 ...
 *
 * It uses a counting semaphore but the optimization is that we may not ever use
 * the semaphore API if there are not simultaneous users of the lock
 *
...
/// Releases the lock and unblocks potential users blocked on
/// leightweight_lock__acquire()
