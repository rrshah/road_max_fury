 ...
 *
 * The library is based on static memory allocation and the string memory will
 * not allocate more memory or grow after initialization. Interesting feature is
 * that the string still allows typical index based operations (although not
 * safe):
 *
 * @code
 *   char memory[64];
...
/**
 * Same as sl_string__initialize() but the string will be copied from
 * 'copy_from'
 */
 ...
 *
 * @note
 * The 'n' style API, namely snprintf() is not provided because all sl_string
 * printf() style API is bounds protected
 */
 ...
 * If you want to get pointers separating the stringing such as "hello world
 * 123", then you can use this function, however, your original stringing will
 * be destroyed.
 * @code
...
/**
 * @returns true if string begins with 'begin_with' but will compare only
 * max_chars_of_begin_with_to_compare
 */
 ...
 *  - First call to this function will set *erased_int to 123, and string will
 * be " abc 1234"
 *  - Second call to this function will set *erased_int to 456, and set the
 * string to ""
 */
 ...
 *
 * @returns True if was able to successfully replace or if it can not find
 * `replace`, otherwise false
 *
 * The use case for these functions are: I want to replace first/last/all of the
 * occurrences of `replace` with `replace_with`. If replace doesn't exist you've
 * still accomplished this. Thus the function will return true even if it can
 * not find `replace` in `string`.
 */
