 ...
 * If RTOS is being utilized, you can use uart__enable_queues() to enable event
 * driven implementation
 *
 * @param peripheral_clock Provide the clock frequency the peripheral is running
 * at in order to set the baud_rate
 * @note Once UART is initialied to a baud rate, this API will not allow further
 * changes to it
 */
...
/**
 * If the RTOS is going to be used, then enabling the queues provides event
 * driven implementation such that uart_get() and uart__put() operate off of
 * queues and you do not have to resort to using polled (cpu eating) version
 *
 * The transmit queue will hold data, such that uart__put() can deposit data and
 * return immediately rather than poll. Likewise, the received data is stored to
 * the receive queue, such that uart__get() will sleep for reception.
 *
